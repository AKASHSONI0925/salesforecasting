...
def predict():
    ...
    if request.method == "POST":
       #get form data
        ...
        try:
            prediction = preprocessDataAndPredict(temperature,fuel_price, cpi ,unemployment, size ,holiday, type_b,type_c)
...
def preprocessDataAndPredict(temperature,fuel_price, cpi ,unemployment, size ,holiday, type_b,type_c):
    ...
    test_data = [float(temperature),float(fuel_price),float(cpi) ,float(unemployment),int( size) ,int(holiday), int(type_b),int(type_c)]
    print(test_data)
